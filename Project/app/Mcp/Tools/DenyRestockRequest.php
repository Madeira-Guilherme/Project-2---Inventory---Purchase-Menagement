...
class DenyRestockRequest extends Tool
{
    ...
    public function handle(Request $request): Response
    {
        ...
        // Prevent updating if already denied or accepted
        if (in_array($restockRequest->status, ['denied', 'completed'])) {
            return Response::json([
                'success' => false,
                'message' => "Cannot complete a request that is already {$restockRequest->status}.",
            ]);
        }
